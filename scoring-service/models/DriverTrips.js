...
// Method to check if driver is available for a given period
driverTripsSchema.methods.isAvailable = function(startDate, endDate) {
...
driverTripsSchema.methods.addTrip = function(tripId, startDate, endDate) {
  if (!this.isAvailable(startDate, endDate)) {
    throw new Error('Driver is not available for the requested time period');
  }
...
driverTripsSchema.index({ email: 1 });
driverTripsSchema.index({ 'trips.start_date': 1, 'trips.end_date': 1 });
